...
function LandingRate({ baseCur = "AUD", cur = "PKR" }) {
...
function Landing({ onStart, onLogin }) {
  return (
    <div className="cd-ld">
      ...
      <p className="cd-ld-sub">One app compares every provider and routes your money the smartest way. Flat 1%. Nothing hidden.</p>
      ...
      <div className="cd-ld-mock">
        <div className="cd-ld-mock-line"><span>You send</span><b>AUD 500.00</b></div>
        <div className="cd-ld-mock-line big"><span>They receive</span><b>PKR 89,896</b></div>
        ...
        <div className="cd-ld-mock-save">AUD 4.20 cheaper than the next option</div>
      </div>
...
export default function ConduitApp() {
  ...
  if (authStage === "landing") {
    return (<div className="cd-root"><style>{css}</style><Landing onStart={() => setAuthStage("signup")} onLogin={() => setAuthStage("login")} /><div className="cd-caption">Conduit · Money moves. Brains guide it.</div></div>);
  }
  if (authStage === "login") {
...
function RateWatch({ recipients, baseCur = "AUD" }) {
  const API = import.meta.env.VITE_API_BASE || "";
  const corridors = [...new Set([...(recipients || []).map(r => r.cur), "PKR"])].filter(Boolean).slice(0, 4);
