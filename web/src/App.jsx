...
import {
  ...
  Landmark, Smartphone, Camera, FileText, Settings, LogOut, Bell, Lock, Mail
} from "lucide-react";
      ...
      <div className="cd-onb-card">
        ...
        {step === 1 && (
          <>
            ...
            <input className="cd-input" placeholder="Street, suburb, postcode" value={addr} onChange={e => setAddr(e.target.value)} />
...
function SendFlow({ profile, recipients, setRecipients, setHistory, goActivity, initialRecipient = null, initialFrom = null, onExit }) {
  ...
  function pickTo(d) { setTo(d); push("user", `${d.flag} ${d.country}`); push("agent", "How should they receive it?"); setStep("method"); }
    ...
    <div className="cd-flow">
      ...
      {step === "recipientPick" && (
        <div className="cd-panel">
          ...
          <button className="cd-opt" onClick={() => { push("user", "Someone new"); push("agent", "Which country is the money going to?"); setStep("to"); }}>
      ...
      {step === "details" && (
        <div className="cd-panel"><div className="cd-form">
          ...
          <input className="cd-input" placeholder={method === "Wallet" ? "0x… or wallet ID" : method === "Cash" ? "+00 000 000 000" : "Account number"} value={rcpDetail} onChange={e => setRcpDetail(e.target.value)} />
          ...
          <button className="cd-primary full" onClick={submitDetails} disabled={!rcpName.trim() || !rcpDetail.trim()}>Continue</button>
        </div></div>
      )}
    ...
    <div className="cd-screen">
      ...
      {adding ? (
        <div className="cd-form card">
          ...
          <label className="cd-fl">{method === "Wallet" ? "Wallet address" : method === "Cash" ? "Mobile number" : "Account / IBAN"}</label>
          <input className="cd-input" placeholder={method === "Wallet" ? "0x… or wallet ID" : method === "Cash" ? "+00 000 000 000" : "Account number"} value={detail} onChange={e => setDetail(e.target.value)} />
          <div className="cd-formrow"><button className="cd-ghost" onClick={() => setAdding(false)}>Cancel</button><button className="cd-primary" onClick={save} disabled={!name.trim() || !detail.trim() || (customMode && !cName.trim())}>Save recipient</button></div>
        </div>
...
.cd-frame{width:100%;max-width:412px;background:#fff;border-radius:30px;overflow:hidden;display:flex;flex-direction:column;height:772px;max-height:91vh;box-shadow:0 30px 80px rgba(0,0,0,.45),0 0 0 1px rgba(255,255,255,.04);}
...
.cd-opt{display:flex;align-items:center;gap:12px;background:#fff;border:1px solid ${C.line};border-radius:14px;padding:13px 15px;cursor:pointer;text-align:left;transition:.15s;animation:rise .3s ease both;}
...
.cd-chip{display:flex;align-items:center;gap:8px;background:#fff;border:1px solid ${C.line};border-radius:13px;padding:13px 12px;cursor:pointer;font-weight:600;color:${C.charcoal};font-size:13.5px;transition:.15s;animation:rise .3s ease both;}
...
.cd-primary{flex:1;background:linear-gradient(135deg,${C.teal},${C.sea});color:#fff;border:none;border-radius:12px;padding:13px;font-weight:700;font-size:14px;cursor:pointer;transition:.15s;}
.cd-primary.full{width:100%;margin-top:10px;}
.cd-primary:disabled{opacity:.4;cursor:default;}
.cd-primary:not(:disabled):hover{transform:translateY(-1px);box-shadow:0 8px 18px rgba(2,128,144,.28);}
...
.cd-hero{text-align:left;background:linear-gradient(160deg,#f4fbfa 0%,#ffffff 60%);border:1.5px solid ${C.mint};border-radius:18px;padding:16px;cursor:pointer;box-shadow:0 12px 30px rgba(2,195,154,.18);transition:.16s;animation:rise .3s ease both;}
...
.cd-save{margin-top:12px;background:#fff6e6;color:#92600a;font-weight:700;font-size:12.5px;padding:9px 12px;border-radius:11px;display:flex;align-items:center;gap:6px;}
.cd-hero-cta{margin-top:12px;background:linear-gradient(135deg,${C.teal},${C.sea});color:#fff;font-weight:700;font-size:14px;border-radius:12px;padding:13px;display:flex;align-items:center;justify-content:center;gap:7px;}
.cd-other-lbl{color:${C.muted};font-weight:700;font-size:11.5px;text-transform:uppercase;letter-spacing:.5px;margin:8px 2px 0;}
.cd-rowcard{display:flex;align-items:center;gap:11px;text-align:left;background:#fff;border:1px solid ${C.line};border-radius:14px;padding:12px 13px;cursor:pointer;transition:.15s;animation:rise .3s ease both;}
...
.cd-send{background:linear-gradient(135deg,${C.teal},${C.sea});color:#fff;border:none;border-radius:15px;padding:16px;font-weight:700;font-size:14.5px;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;transition:.16s;box-shadow:0 8px 20px rgba(2,128,144,.28);}
...
.cd-greet-sub{color:${C.muted};font-size:13.5px;margin-top:-12px;}
.cd-bigsend{display:flex;align-items:center;gap:13px;background:linear-gradient(135deg,${C.teal},${C.sea});border:none;border-radius:18px;padding:17px 16px;cursor:pointer;text-align:left;box-shadow:0 12px 28px rgba(2,128,144,.3);transition:.16s;}
...
.cd-limit-mini{background:#fff;border:1px solid ${C.line};border-radius:15px;padding:14px;}
