...
const METHOD_ICON = { Bank: Building2, Cash: Banknote };
const METHOD_LABEL = { Bank: "Bank account", Cash: "Cash pickup" };
...
function validateDetail(method, token, value) {
  ...
  if (!v) return { level: "idle", msg: "" };
  {
    ...
    return { level: "warn", msg: "Double-check this detail is correct" };
  }
  if (method === "Cash") {
...
/* ---------------- Analytics: PostHog. No-ops safely if no key is set. ----------------
 * Answers the questions that decide strategy:
 *   - Which corridors do people actually use?
 *   - Do they get to a comparison?
 *   - Do they click through to a provider?  <- the money event
 * Never sends names, emails, or account details. Corridors and amounts only.
 */
...
function initAnalytics() {
  if (!PH_KEY || phReady || typeof window === "undefined") return;
  try {
    // Official PostHog snippet (loader), trimmed.
    ...
    window.posthog.init(PH_KEY, { api_host: PH_HOST, person_profiles: "identified_only", capture_pageview: true });
    phReady = true;
  } catch (e) { /* analytics must never break the app */ }
}

function track(event, props = {}) {
  try { if (PH_KEY && window.posthog?.capture) window.posthog.capture(event, props); } catch (e) {}
}
function identify(id) {
  try { if (PH_KEY && window.posthog?.identify) window.posthog.identify(id); } catch (e) {}
}
  ...
  return (
    <div className="cd-ld">
      ...
      <div className="cd-ld-props">
        ...
        <div className="cd-ld-prop"><b>Bank deposit or cash pickup.</b><span>Your recipient chooses how the money lands — some options arrive in minutes.</span></div>
        <div className="cd-ld-prop"><b>No sign-up wall. Just compare.</b><span>See every provider's real cost in seconds — no ID needed.</span></div>
      </div>
...
export default function ConduitApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!sess?.access_token) { setAuthStage("landing"); track("landing_viewed"); return; }
      ...
      if (!user?.id) { clearSession(); setAuthStage("landing"); track("landing_viewed"); return; }
      identify(user.id);
  ...
  async function completeOnboarding(p) {
    track("signup_completed", { country: p.country });
    setProfile(p);
    setAuthStage("in");
  ...
  if (authStage === "landing") {
    return (<div className="cd-root"><style>{css}</style><Landing onStart={() => { track("signup_started"); setAuthStage("signup"); }} onLogin={() => setAuthStage("login")} onLegal={(t) => setLegalTab(t)} /><div className="cd-caption">Conduit · Money moves. Brains guide it.</div></div>);
  }
  if (authStage === "login") {
  ...
  if (!profile) {
    return (<div className="cd-root"><style>{css}</style><Onboarding onDone={completeOnboarding} onLogin={() => setAuthStage("login")} hasSession={Boolean(USER_TOKEN)} startEmail={""} /><div className="cd-caption">Conduit · interactive prototype</div></div>);
  }
...
function Onboarding({ onDone, onLogin = () => {}, hasSession = false, startEmail = "" }) {
  ...
  const steps = ["Account", "Details", "Ready"];

  function finish() {
    const initials = (name.trim() || "You").split(/\s+/).map(w => w[0]).slice(0, 2).join("").toUpperCase();
    onDone({ name: name.trim() || "New user", email: email.trim() || "you@email.com", country, initials, tier: "Tier 1", status: "Unverified", limitMax: 15000, currency: COUNTRY_SRC[country] || "USD" });
  }
      ...
      <div className="cd-onb">
        ...
        <div className="cd-onb-card welcome">
          ...
          <div className="cd-vprops">
            ...
            <div className="cd-vprop"><span className="cd-vprop-ic"><Zap size={18} color={C.teal} /></span><span className="cd-vprop-txt">Bank deposit or cash pickup — some arrive in minutes</span></div>
            <div className="cd-vprop"><span className="cd-vprop-ic"><ShieldCheck size={18} color={C.teal} /></span><span className="cd-vprop-txt">Trusted providers. No ID needed to compare.</span></div>
          </div>
      ...
      <div className="cd-onb-card">
        ...
        {step === 2 && (
          <div className="cd-review">
            <div className="cd-verified-ic"><Sparkles size={38} color="#fff" /></div>
            <h3 className="cd-onb-h center">You're all set</h3>
            <p className="cd-onb-p center">Conduit compares every provider and points you to the best deal. We never hold your money — you finish each transfer on the provider's own site.</p>
            <div className="cd-rev-rows">
              ...
              <div className="cd-rev"><span>Sending from</span><span>{country}</span></div>
            </div>
            <p className="cd-onb-p center" style={{ marginTop: 14, fontSize: 12 }}>No ID needed to compare. You can add a verified badge later in your profile.</p>
          </div>
        )}

        {!(sbOn() && step === 0) && (
          <button className="cd-primary full big" disabled={!canNext} onClick={() => step < 2 ? setStep(step + 1) : finish()}>
            {step === 2 ? "Enter Conduit" : "Continue"}
          </button>
        )}
...
/* ============================== SEND TAB (home + flow) ============================== */
/* ---------------- Send Moment: the agent's core job ----------------
 * Answers the question no other remittance app answers:
 *   "Is right now a good time to send, and through whom?"
 *
 * HONESTY RULE: we never predict. We state observable facts — where today's
 * rate sits against the last 30 days, and who is genuinely cheapest right now.
 */
function SendMoment({ profile, recipients, history, onQuick }) {
  const API = import.meta.env.VITE_API_BASE || "";
  const [rates, setRates] = useState(null);
  const [best, setBest] = useState(null);
  const [err, setErr] = useState(false);

  // Who does this person send to, and how much? Use their real behaviour.
  const lastSend = history?.[0];
  const target = recipients?.find(r => r.name === lastSend?.recipientName) || recipients?.[0];
  const usual = lastSend?.amount || 400;
  const base = profile?.currency || "USD";

  useEffect(() => {
    if (!target) return;
    let dead = false;
    setRates(null); setBest(null); setErr(false);
    (async () => {
      try {
        const [r, q] = await Promise.all([
          fetch(`${API}/api/rates?from=${base}&to=${target.cur}`).then(x => x.json()),
          compareQuotes({ fromCur: base, toCur: target.cur, amount: usual, method: target.method || "Bank" }),
        ]);
        if (dead) return;
        if (!r?.rate) { setErr(true); return; }
        setRates(r);
        setBest(q?.quotes?.[0] || null);
      } catch (e) { if (!dead) setErr(true); }
    })();
    return () => { dead = true; };
  }, [target?.id, usual, base]);
  ...
  if (!rates || !best) {
    return (
      <div className="cd-sm loading">
        <span className="cd-spin" /> <span>Checking the best moment to send…</span>
      </div>
    );
  }

  const v = rates.verdict; // "good" | "average" | "below"
  const vsAvg = Math.abs(rates.vsAvg).toFixed(1);
  const receives = best.received;
  // What the same amount would have got them at the 30-day average.
  const atAvg = receives * (rates.avg30 / rates.rate);
  const diff = receives - atAvg;

  const headline = v === "good" ? "Good moment to send"
    : v === "below" ? "Rate is below its usual"
    : "Rate is about normal";

  const line = v === "good"
    ? `The ${target.cur} is weak — ${vsAvg}% above its 30-day average. ${target.name.split(" ")[0]} gets more than usual right now.`
    : v === "below"
    ? `Today is ${vsAvg}% below the 30-day average. If you can wait, it may be worth it — but here's the best option today.`
    : `Today is in line with the last 30 days. Here's the cheapest way to send right now.`;

  return (
    <div className={`cd-sm ${v}`}>
      <div className="cd-sm-top">
        <span className="cd-sm-eyebrow"><Sparkles size={12} /> Send moment</span>
        <span className={`cd-sm-pill ${v}`}>{v === "good" ? "Better than usual" : v === "below" ? "Below average" : "Normal"}</span>
      </div>

      <div className="cd-sm-head">{headline}</div>
      <div className="cd-sm-line">{line}</div>

      <div className="cd-sm-box">
        <div className="cd-sm-row">
          <span className="cd-sm-k">You send</span>
          <span className="cd-sm-v">{base} {fmt(usual, base)}</span>
        </div>
        <div className="cd-sm-row big">
          <span className="cd-sm-k">{target.name.split(" ")[0]} receives</span>
          <span className="cd-sm-v">{target.cur} {fmt(receives, target.cur)}</span>
        </div>
        {v === "good" && diff > 0 && (
          <div className="cd-sm-gain">+{fmt(diff, target.cur)} {target.cur} more than a typical day this month</div>
        )}
        <div className="cd-sm-prov">
          <span className="cd-prov-badge sm2" style={{ background: best.p.color, color: best.p.textColor || "#fff" }}>{best.p.initials}</span>
          <span>Cheapest right now: <b>{best.p.name}</b>{best.p.instant ? " · arrives in minutes" : ` · ${best.p.speed}`}</span>
        </div>
      </div>

      <button className="cd-sm-cta" onClick={() => onQuick(target)}>
        Send to {target.name.split(" ")[0]} <ArrowRight size={16} />
      </button>
      <div className="cd-sm-foot">We compare today against the last 30 days. We don't predict rates — nobody can.</div>
    </div>
  );
}
...
function RateWatch({ recipients, baseCur = "AUD" }) {
  ...
  function setAlert() {
    ...
    track("rate_alert_set", { corridor: `${baseCur}-${cur}`, target: t });
  }
  function clearAlert() {
...
function Home({ profile, recipients, history, onSend, onQuick, goActivity, goPeople, goProfile }) {
  ...
  return (
    <div className="cd-home">
      ...
      <SendMoment profile={profile} recipients={recipients} history={history} onQuick={onQuick} />
...
function SendFlow({ profile, recipients, setRecipients, setHistory, addRecipient, addTransfer, goActivity, initialRecipient = null, initialFrom = null, onExit }) {
  ...
  useEffect(() => { track("send_started", { quick: Boolean(quick) }); }, []);
  ...
  function pickSaved(r) {
    ...
    setRcpName(r.name); setRcpDetail(r.detail);
  ...
  function pickMethod(m) {
    ...
    push("agent", m === "Cash" ? "Recipient's name and mobile for pickup?" : "Recipient's name and account number?");
    setStep("details");
  }
  function submitDetails() {
    ...
    remember();
    push("user", `${rcpName} · ${shortDetail(rcpDetail)}`);
  ...
  async function submitAmount(val) {
    ...
    try {
      const dc = to.cur;
      const { quotes: qs } = await compareQuotes({ fromCur: from.code, toCur: to.cur, amount: a, method });
      if (!qs || !qs.length) throw new Error("No providers available for this route.");
      setQuotes(qs); setStep("results");
      track("quotes_viewed", {
        corridor: `${from.code}->${to.cur}`,
        from_currency: from.code, to_country: to.country, to_currency: to.cur,
        amount: a, method, quote_count: qs.length,
        best_provider: qs[0]?.p?.name, best_is_instant: Boolean(qs[0]?.p?.instant),
      });
  ...
  function trackProviderPick(q) {
    track("provider_selected", {
      provider: q?.p?.name, corridor: `${from?.code}->${to?.cur}`,
      amount, instant: Boolean(q?.p?.instant), monetized: Boolean(q?.p?.monetized),
    });
  }
  function confirmSend() {
    ...
    track("handoff_clicked", {
      provider: chosen.p.name, corridor: `${from.code}->${to.cur}`,
      amount, received: chosen.received, method,
      instant: Boolean(chosen.p.instant), monetized: Boolean(chosen.p.monetized),
    });
    if (chosen.p.url) window.open(chosen.p.url, "_blank", "noopener,noreferrer");
    if (!savedRcp && saveNew && rcpName.trim()) addRecipient({ id: "r" + Date.now(), name: rcpName.trim(), country: to.country, cur: to.cur, flag: to.flag, method, detail: rcpDetail });
  ...
  return (
    <div className="cd-flow">
      ...
      {step === "method" && (
        <div className="cd-panel">
          {["Bank", "Cash"].map(m => { const Ic = METHOD_ICON[m]; return (
            <button key={m} className="cd-opt" onClick={() => pickMethod(m)}>
              <span className="cd-methodicon"><Ic size={17} color="#fff" /></span>
              <span className="cd-opt-main">{METHOD_LABEL[m]}<em className="cd-opt-em">{m === "Cash" ? "500k+ agents · minutes" : "Direct deposit · as fast as 5 min"}</em></span>
      ...
      {step === "details" && (
        <div className="cd-panel"><div className="cd-form">
          <label className="cd-fl">Recipient name</label><input className="cd-input" placeholder="Full name" value={rcpName} onChange={e => setRcpName(e.target.value)} />
                    <label className="cd-fl">{method === "Cash" ? "Mobile number" : "Account / IBAN"}</label>
          <input className={`cd-input ${rcpDetail.trim() ? `dv-${validateDetail(method, token, rcpDetail).level}` : ""}`} placeholder={method === "Cash" ? "+00 000 000 000" : "Account number or IBAN"} value={rcpDetail} onChange={e => setRcpDetail(e.target.value)} />
      ...
      {step === "results" && (() => {
        ...
        return (
          <div className="cd-results" ref={resultsRef}>
            ...
            <button className="cd-hero" onClick={() => { trackProviderPick(best); setChosen(best); setStep("confirm"); }}>
              <div className="cd-hero-top">
                ...
                <div className="cd-badges">{best.p.instant && <span className="cd-badge inst"><Zap size={11} /> Instant</span>}{best.live && <span className="cd-badge live">Live rate</span>}<span className="cd-badge best">Best value</span>{bestFast && !best.p.instant && <span className="cd-badge fast"><Zap size={11} /> Fastest</span>}</div>
              </div>
            ...
            {rest.map(q => { const isFast = q.p.id === fastestId; return (
              <button key={q.p.id} className="cd-rowcard" onClick={() => { trackProviderPick(q); setChosen(q); setStep("confirm"); }}>
                <span className="cd-prov-badge" style={{ background: q.p.color, color: q.p.textColor || "#fff" }}>{q.p.initials}</span>
                <div className="cd-rc-main"><div className="cd-rc-name">{q.p.name}{q.p.instant && <span className="cd-badge inst sm"><Zap size={10} /> Instant</span>}{q.live && <span className="cd-badge live sm">Live</span>}{isFast && !q.p.instant && <span className="cd-badge fast sm"><Zap size={10} /> Fastest</span>}</div><div className="cd-rc-sub"><Clock size={12} /> {q.p.speed} · Cost {from.code} {fmt(q.totalCost, from.code)}</div></div>
      ...
      {step === "confirm" && chosen && (
        <div className="cd-confirm" ref={confirmRef}>
          ...
          <div className="cd-sum">
            ...
            <div className="cd-recipient"><div className="cd-rcp-ava">{to.flag}</div><div><div className="cd-rcp-name">{rcpName || `Recipient in ${to.country}`}</div><div className="cd-rcp-sub">{to.country} · {`${METHOD_LABEL[method]}${rcpDetail ? " · " + shortDetail(rcpDetail) : ""}`}</div></div></div>
          </div>
...
function People({ recipients, setRecipients, addRecipient, removeRecipient }) {
  ...
  function save() {
    ...
    addRecipient({ id: "r" + Date.now(), name: name.trim(), country: dest.country, cur: dest.cur, flag: dest.flag, method, detail });
    ...
    <div className="cd-screen">
      ...
      {adding ? (
        <div className="cd-form card">
          ...
          <div className="cd-toggle">{["Bank", "Cash"].map(m => <button key={m} className={`cd-tog ${method === m ? "on" : ""}`} onClick={() => setMethod(m)}>{METHOD_LABEL[m].split(" ")[0]}</button>)}</div>
                    <label className="cd-fl">{method === "Cash" ? "Mobile number" : "Account / IBAN"}</label>
          <input className={`cd-input ${detail.trim() ? `dv-${validateDetail(method, token, detail).level}` : ""}`} placeholder={method === "Cash" ? "+00 000 000 000" : "Account number or IBAN"} value={detail} onChange={e => setDetail(e.target.value)} />
        ...
        : (<div className="cd-list">{recipients.map(r => { const Ic = METHOD_ICON[r.method]; return (
          <div key={r.id} className="cd-rrow"><span className="cd-flag big">{r.flag}</span><div className="cd-rmain"><div className="cd-rname">{r.name}</div><div className="cd-rsub"><Ic size={12} /> {r.country} · shortDetail(r.detail)</div></div><button className="cd-del" onClick={() => removeRecipient(r.id)}><Trash2 size={15} /></button></div>); })}</div>)}
    </div>
  );
}
...
/* ============================== PROFILE ============================== */
function VerifyCard({ profile }) {
  const API = import.meta.env.VITE_API_BASE || "";
  const [status, setStatus] = useState(profile.status === "Verified" ? "verified" : "idle");
  const [err, setErr] = useState("");
  const pollRef = useRef({ stop: false });
  ...
  async function startVerify() {
    ...
    try {
      ...
      const d = await r.json();
      if (!d.url) throw new Error(d.error || "Could not start verification");
      window.open(d.url, "_blank", "noopener,noreferrer");
      setStatus("pending");
      poll(d.id);
    } catch (e) { setStatus("failed"); setErr(String(e.message || e)); }
  }
  ...
  function poll(id) {
    ...
    const tick = async () => {
      ...
      try {
        const r = await fetch(`${API}/api/identity/status?id=${encodeURIComponent(id)}`);
        const d = await r.json();
        if (d.status === "verified") { setStatus("verified"); return; }
        if (d.status === "canceled") { setStatus("failed"); setErr("Verification was canceled"); return; }
      } catch (e) { /* transient */ }
      if (!pollRef.current.stop) setTimeout(tick, 3500);
    };
    setTimeout(tick, 3500);
  ...
  if (status === "verified") {
    return (
      <div className="cd-card2 cd-vfy done">
        <div className="cd-vfy-row">
          <span className="cd-vfy-ic on"><BadgeCheck size={20} color="#0a7d52" /></span>
          <div className="cd-vfy-txt"><b>Verified sender</b><span>Your identity is confirmed with Stripe.</span></div>
        </div>
      </div>
    );
  }
  return (
    <div className="cd-card2 cd-vfy">
      <div className="cd-vfy-row">
        <span className="cd-vfy-ic"><ShieldCheck size={20} color={C.teal} /></span>
        <div className="cd-vfy-txt">
          <b>Get a verified badge <span className="cd-vfy-opt">Optional</span></b>
          <span>You don't need this to compare or send — providers run their own checks. Verifying adds a trust badge to your profile, and lets us pass your verification to partner providers as we add them.</span>
        </div>
      </div>
      {status === "pending" ? (
        <div className="cd-vfy-pending"><span className="cd-spin" /> Waiting for you to finish in the Stripe tab…</div>
      ) : (
        <button className="cd-vfy-btn" onClick={startVerify} disabled={status === "opening"}>
          {status === "opening" ? "Starting secure check…" : "Verify with Stripe"}
        </button>
      )}
      {status === "failed" && <div className="cd-valmsg error" style={{ marginTop: 8 }}><X size={13} /> {err} — <span style={{ textDecoration: "underline", cursor: "pointer" }} onClick={startVerify}>try again</span></div>}
    </div>
  );
}
  ...
  return (
    <div className="cd-screen">
      <div className="cd-prof-top">
        ...
        {profile.status === "Verified" ? (<div className="cd-prof-verified"><BadgeCheck size={14} /> Verified sender</div>) : (<div className="cd-prof-verified plain">{profile.tier}</div>)}
      </div>
      ...
      <div className="cd-card2">
        ...
        <div className="cd-bar"><div className="cd-bar-fill" style={{ width: pct + "%" }} /></div>
        <div className="cd-card2-sub">A guide only — each provider sets its own limits.</div>
      </div>

      <VerifyCard profile={profile} />
      ...
      <div className="cd-list">
        <SetRow icon={ShieldCheck} label="Identity & verification" value={profile.status === "Verified" ? "Verified" : "Optional"} />
...
.cd-sm{border-radius:20px;padding:17px;margin-top:16px;border:1px solid ${C.line};background:#fff;box-shadow:0 1px 2px rgba(11,32,39,.04),0 16px 34px -22px rgba(11,32,39,.18);}
.cd-sm.loading{display:flex;align-items:center;gap:10px;color:${C.muted};font-size:12.5px;font-weight:600;}
.cd-sm.good{background:linear-gradient(165deg,#f2fbf9,#ffffff 60%);border-color:#cdeee6;}
.cd-sm.below{background:linear-gradient(165deg,#fffaf0,#ffffff 60%);border-color:#f0dcae;}
.cd-sm-top{display:flex;justify-content:space-between;align-items:center;margin-bottom:13px;}
.cd-sm-eyebrow{display:inline-flex;align-items:center;gap:6px;color:${C.teal};font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:1.1px;}
.cd-sm-pill{font-size:10px;font-weight:800;padding:4px 9px;border-radius:999px;background:#eef4f5;color:${C.muted};}
.cd-sm-pill.good{background:linear-gradient(135deg,${C.mint},${C.sea});color:#fff;}
.cd-sm-pill.below{background:#fdf3df;color:#92600a;}
.cd-sm-head{font-size:18px;font-weight:800;color:${C.ink};letter-spacing:-.4px;}
.cd-sm-line{color:${C.muted};font-size:12.5px;line-height:1.55;margin-top:6px;}
.cd-sm-box{margin-top:14px;background:#fbfdfd;border:1px solid #eef3f4;border-radius:15px;padding:13px;}
.cd-sm-row{display:flex;justify-content:space-between;align-items:baseline;padding:3px 0;}
.cd-sm-k{color:${C.muted};font-size:12px;font-weight:600;}
.cd-sm-v{font-size:14px;font-weight:700;color:${C.charcoal};}
.cd-sm-row.big .cd-sm-v{font-size:20px;font-weight:800;color:${C.ink};letter-spacing:-.4px;}
.cd-sm-gain{margin-top:7px;background:linear-gradient(135deg,#e9f8f5,#f4fcfa);border:1px solid #cdeee6;color:#0a7d52;font-weight:700;font-size:11.5px;padding:8px 10px;border-radius:10px;text-align:center;}
.cd-sm-prov{display:flex;align-items:center;gap:10px;margin-top:12px;padding-top:11px;border-top:1px solid #eef3f4;color:${C.muted};font-size:12px;}
.cd-sm-prov b{color:${C.charcoal};font-weight:700;}
.cd-prov-badge.sm2{width:24px;height:24px;font-size:9.5px;border-radius:7px;flex-shrink:0;}
.cd-sm-cta{width:100%;margin-top:14px;display:flex;align-items:center;justify-content:center;gap:8px;background:linear-gradient(135deg,${C.teal},${C.sea});border:none;color:#fff;font-weight:800;font-size:14px;padding:14px;border-radius:14px;cursor:pointer;box-shadow:0 10px 22px -8px rgba(2,128,144,.45);transition:.15s;}
.cd-sm-cta:hover{transform:translateY(-1px);}
.cd-sm-foot{margin-top:10px;color:#93a6ab;font-size:10.5px;text-align:center;line-height:1.45;}
...
.cd-badge.inst{background:linear-gradient(135deg,#02C39A,#00A896);color:#fff;}
.cd-badge.inst.sm{margin-left:8px;font-size:8.5px;padding:2px 6px;vertical-align:2px;}
...
.cd-vfy-row{display:flex;align-items:flex-start;gap:14px;}
.cd-vfy-ic{width:42px;height:42px;border-radius:13px;background:linear-gradient(135deg,#eef8f9,#f7fcfc);border:1px solid #d8ecee;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
.cd-vfy-ic.on{background:linear-gradient(135deg,#e9f8f5,#f4fcfa);border-color:#cdeee6;}
.cd-vfy-txt b{display:flex;align-items:center;gap:8px;font-size:14px;color:${C.ink};letter-spacing:-.2px;}
.cd-vfy-txt span{display:block;color:${C.muted};font-size:12px;margin-top:5px;line-height:1.5;}
.cd-vfy-opt{background:#eef4f5;color:${C.muted};font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.6px;padding:3px 7px;border-radius:999px;}
.cd-vfy-btn{width:100%;margin-top:13px;background:#fff;border:1.5px solid ${C.sea};color:${C.teal};font-weight:700;font-size:13px;padding:12px;border-radius:13px;cursor:pointer;transition:.15s;}
.cd-vfy-btn:hover{background:#f3fbfa;}
.cd-vfy-btn:disabled{opacity:.6;cursor:default;}
.cd-vfy-pending{margin-top:13px;display:flex;align-items:center;justify-content:center;gap:9px;color:${C.muted};font-size:12.5px;font-weight:600;padding:12px;background:${C.cloud};border-radius:13px;}
.cd-prof-verified.plain{background:#eef4f5;color:${C.muted};}
...
.cd-legal{width:100%;max-width:640px;margin:0 auto;padding:26px 22px 30px;text-align:left;background:#fff;border-radius:22px;box-shadow:0 30px 70px -30px rgba(0,0,0,.5);}
