...
function LoginScreen({ onLoggedIn, onNewUser }) {
  ...
  return (
    <div className="cd-frame cd-onbframe">
      <div className="cd-onb-hero"><Logo size={44} /><div className="cd-onb-brand">Conduit</div><div className="cd-tag" style={{ textAlign: "center" }}>Money moves. Brains guide it.</div></div>
      <div className="cd-onb-card">
        ...
        {state === "sent" ? (
          <>
            ...
            <p className="cd-onb-p" style={{ marginTop: 10 }}>We sent a sign-in link to <b>{email.trim()}</b>. Open it on this device and you'll land back here, signed in. (Give it a minute — check spam too.)</p>
          </>
        ) : (
          <>
            <p className="cd-onb-p">Enter your email and we'll send you a secure sign-in link. No password needed.</p>
            <div className="cd-form">
              <label className="cd-fl">Email</label>
              <input className="cd-input" type="email" placeholder="you@email.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} />
              {state === "error" && <div className="cd-valmsg error"><X size={13} /> {err}</div>}
              <button className="cd-primary full" onClick={send} disabled={state === "sending"}>{state === "sending" ? "Sending link…" : "Email me a sign-in link"}</button>
            </div>
          </>
        )}
        <p className="cd-onb-p" style={{ marginTop: 14 }}>New to Conduit? <span style={{ color: "#028090", fontWeight: 700, cursor: "pointer" }} onClick={onNewUser}>Create your account</span></p>
      </div>
    </div>
  );
}
...
export default function ConduitApp() {
  ...
  const [authStage, setAuthStage] = useState(sbOn() ? "checking" : "open"); // checking | login | signup | in | open
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!sess?.access_token) { setAuthStage("login"); return; }
      const user = await fetchAuthedUser(sess.access_token);
      if (!user?.id) { clearSession(); setAuthStage("login"); return; }
      setUserToken(sess.access_token);
      try {
        const p = await sbGetProfile();
        if (p) {
          setProfile({ name: p.name, email: p.email || user.email, country: p.country, initials: (p.name || "U").split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase(), tier: "Tier 1", status: p.verified ? "Verified" : "Pending", limitMax: 15000, currency: "AUD" });
          setAuthStage("in");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (rcps.length === 0) {
          const seeds = [
            { name: "Amir Khan", country: "Pakistan", method: "Bank", detail: "PK36SCBL0000001123456702", flag: "🇵🇰" },
            { name: "Maria Santos", country: "Philippines", method: "Cash", detail: "+63 917 555 0148", flag: "🇵🇭" },
          ];
          const inserted = [];
  ...
  if (authStage === "checking") {
    return (<div className="cd-root"><style>{css}</style><div className="cd-frame cd-onbframe" style={{ alignItems: "center", justifyContent: "center", display: "flex" }}><div style={{ textAlign: "center" }}><Logo size={44} /><p className="cd-onb-p" style={{ marginTop: 12 }}>Signing you in…</p></div></div></div>);
  }
  if (authStage === "login") {
    return (<div className="cd-root"><style>{css}</style><LoginScreen onLoggedIn={() => {}} onNewUser={() => setAuthStage("signup")} /><div className="cd-caption">Conduit · sign in with a magic link</div></div>);
  }
  if (!profile) {
    ...
    <div className="cd-root">
      ...
      <div className="cd-frame">
        ...
        <div className="cd-body">
          {tab === "send" && <SendTab profile={profile} recipients={recipients} setRecipients={setRecipients} setHistory={setHistory} history={history} addRecipient={addRecipient} addTransfer={addTransfer} goActivity={() => setTab("activity")} goPeople={() => setTab("people")} />}
          ...
          {tab === "profile" && <Profile profile={profile} history={history} onSignOut={signOut} />}
        </div>
...
/* ============================== SEND TAB (home + flow) ============================== */
function SendTab({ profile, recipients, setRecipients, setHistory, history, addRecipient, addTransfer, goActivity, goPeople }) {
...
.cd-idverified{display:flex;align-items:center;gap:8px;background:#eafaf3;border:1px solid #b9ecd6;color:#0a7d52;font-weight:700;font-size:13.5px;border-radius:12px;padding:13px 14px;margin-top:4px;}
